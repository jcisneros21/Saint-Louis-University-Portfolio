 ...
 * 
 *    Allocate & myfree memory
 *
 *    Tests mymalloc() and myfree() by repeatedly allocating and myfreeing
 *    chunks of memory of up to size maxblock.
 *
 *    -v is verbose mode, and prints a message with each call to mymalloc/myfree
 *
...
main(int argc, char *argv[])
{
  ...
  long mallocs=0,frees=0;  // number of mymallocs, number of myfrees
  ...
  while (1) {
	...
	if (*(slots[n]+j) != (char) j) {
      ...
      // Going to mymalloc slot[n]
      if (verbose) printf("mymalloc slot %d.\n",n);
